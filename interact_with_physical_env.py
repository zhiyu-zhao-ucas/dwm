...
class EnvParams:
    ...
    num_env: int = 5  # Number of parallel environments
...
def create_encoder_params(env, num_envs, params=None):
    ...
    # feature_inner_dim.extend([env.width, env.height] * env.num_objects)  # For x and y coordinates
...
def main():
    ...
    encoder_params = create_encoder_params(env, num_envs, params)
    ...
    for step in range(num_steps):
        ...
        tensor_obs = {key: torch.tensor(value) for key, value in obs.items()}
        
        # Process observations through encoder
        encoded_next_obs = encoder(tensor_obs)
