...
from fcdl.model.inference_cmi import InferenceCMI
...
def test_policy_evaluation(params, inference, policy, step):
    ...
    with torch.no_grad():
        if params.env_params.env_name == 'Chemical':
            ...
            test_policy_evaluation_physical(params, inference, policy, step)
        else:
            pass
...
def test_policy_evaluation_physical(params, inference, policy, step):
    test_params = params.env_params.physical_env_params.test_params
    # "test1": number of noisy nodes = 2
    # "test2": number of noisy nodes = 4
    # "test3": number of noisy nodes = 6
...
def train(params):
    ...
    # elif isinstance(env, Physical):
    #     torch.save(env.get_save_information(), os.path.join(params.rslts_dir, "physical_env_params"))
    # init model
    ...
    for step in tqdm(range(start_step, total_steps)):
        ...
        if is_vecenv and done.any():
            ...
            obs = env.reset()
            logger.error(f"env.cur_step: {env.cur_step}")
