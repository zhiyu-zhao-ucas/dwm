...
def ood_evaluation(params, inference, obs_batch, actions_batch, next_obses_batch, info_batch, step):
    inference.eval()
    with torch.no_grad():
        if params.env_params.env_name == 'Causal':
...
def ood_evaluation_causal(params, inference, obs_batch, actions_batch, next_obses_batch, info_batch, step):
    ...
    # For each test environment (different complexity or structure)
    ...
    if params.training_params.zero_shot:
        os.makedirs("downstream/Causal/ood_eval", exist_ok=True)
    else:
        os.makedirs("downstream/Causal/ood_eval", exist_ok=True)
    ...
    seed = params.seed
    if params.training_params.zero_shot:
        filename = f"downstream/Causal/ood_eval/{algo}-{seed}.json"
    else:
        filename = f"downstream/Causal/ood_eval/{algo}-{seed}.json"
...
def test_policy_evaluation(params, inference, policy, step):
    ...
    with torch.no_grad():
        if params.env_params.env_name == 'Causal':
            test_policy_evaluation_causal(params, inference, policy, step)
        else:
            pass
...
def test_policy_evaluation_causal(params, inference, policy, step):
    ...
    import os
    video_dir = os.path.join("downstream/Causal/videos")
    ...
    for i, test_param in enumerate(test_params):
        ...
        for test_scale in test_scales:
            ...
            # Record video for the first episode if not in vectorized environment
            should_record = not is_vecenv and episode_num == 0
            ...
            if is_vecenv:
                ...
                pbar = tqdm(total=total_episodes, desc=f"Testing {test_env_name}")
                actions = []
                ...
                if should_record and hasattr(env, 'start_video_recording'):
                    video_path = os.path.join(video_dir, f"{test_env_name}_algo-{params.training_params.inference_algo}_seed-{params.seed}_step-{step}.mp4")
                    logger.info(f"Recording video to {video_path}")
                    env.start_video_recording(video_path)
                    count = 0
                while episode_num < total_episodes:
                    action = policy.act(obs, deterministic=True)
                    next_obs, env_reward, done, info = env.step(action)
                    ...
                    if should_record and hasattr(env, 'add_frame_to_video'):
                        env.add_frame_to_video()
                    if should_record and hasattr(env, 'save_image'):
                        image_path = os.path.join(video_dir, f"step_{count}.png")
                        env.save_image(image_path)
                        count += 1
                    ...
                    if done:
                        ...
                        # if should_record and episode_num == 0 and hasattr(env, 'stop_video_recording'):
                ...
                # Ensure recording is stopped if we exit the loop
                if should_record and hasattr(env, 'stop_video_recording'):
                    env.stop_video_recording()
    ...
    if zero_shot:
        os.makedirs("downstream/Causal/reward", exist_ok=True)
    else:
        os.makedirs("downstream/Causal/reward", exist_ok=True)
    ...
    if not getattr(params, "mute_wandb", False):
        wandb.log(test_detail, step+1)
    inference.encoder.causal_train = True
...
def train(params):
    ...
    env_name = "Causal"  # Force environment to be Causal
    params.env_params.env_name = env_name
    ...
    if hasattr(params.env_params, "causal_env_params"):
        if hasattr(params.env_params.causal_env_params, "model_type"):
            env_specific_type = params.env_params.causal_env_params.model_type
    ...
    # if isinstance(env, Causal):
    #     torch.save(env.get_save_information(), os.path.join(params.rslts_dir, "causal_env_params"))
...
if __name__ == "__main__":
    params = TrainingParams(training_params_fname="causal_video_params.json", train=True)
