...
if __name__ == "__main__":
    # Initialize params from policy_params.json first
    params = TrainingParams(training_params_fname="policy_params.json", train=True)
    
    # Override with command line args
    override_params_from_cli_args(params)
    
    # Extract model path from load_inference path
    if hasattr(params.training_params, "load_inference") and params.training_params.load_inference:
        # The load_inference path is expected to be in the format:
        # data3/iwhwang/causal_rl/chain/compressed_dwm_models/${algo}-${seed}/trained_models/inference_final
        # We need to get the params file from the same directory
        model_path = params.training_params.load_inference
        params_path = os.path.join(os.path.dirname(os.path.dirname(model_path)), "params")
        ...
        if os.path.exists(params_path):
            ...
            original_cuda_id = params.cuda_id
            original_device = getattr(params, "device", None)
            original_mute_wandb = getattr(params.training_params, "mute_wandb", False)
            original_zero_shot = getattr(params.training_params, "zero_shot", False)
            original_load_inference = params.training_params.load_inference
            
            # Update params from loaded file
            for key, value in params_dict.items():
                setattr(params, key, value)
            
            # Restore original values that shouldn't be overridden
            params.seed = original_seed
            params.cuda_id = original_cuda_id
            if original_device:
                params.device = original_device
            params.training_params.mute_wandb = original_mute_wandb
            params.training_params.zero_shot = original_zero_shot
            params.training_params.load_inference = original_load_inference
            
            logger.info(f"Params loaded and merged with command line args")
        else:
            logger.warning(f"Params file not found at {params_path}, using default params")
    
    # Start training with the updated params
    train(params)
