...
def process_model(name, device):
    ...
    model_path = f"dwm_fork_code_size/{name}/trained_models/inference_final"
    reference_model_path = f"dwm_fork_code_size/{reference_model_name}/trained_models/inference_final"
    params_path = f"dwm_fork_code_size/{name}/params"
    env_params_path = f"dwm_fork_code_size/{name}/params"
    ...
    with torch.no_grad():
        ...
        # 保存结果
        results_dir = f"dwm_local_mask_analysis_results_code_size/{name}"
...
def main():
    ...
    if all_high_mean_results:
        ...
        df = pd.DataFrame(data_dict)
        df.to_csv("dwm_local_mask_analysis_results_code_size/best_high_mean_overall.csv", index=False)
